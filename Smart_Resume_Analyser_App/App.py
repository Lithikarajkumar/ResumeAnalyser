...
connection = pymysql.connect(host='localhost', user='root', password='root123')
...
def run():
    st.title("ResumeScout - Smart Resume Analyser")
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'machine_learning_hub' and ad_password == 'mlhub123':
                ...
                fig = px.pie(values=values, names=values.index, title='Predicted Field according to Resume Score')
                st.plotly_chart(fig)

                # Pie chart for User's Experienced Level
                user_level_counts = plot_data['User_level'].value_counts()  # Count occurrences of each user level
                user_level_labels = user_level_counts.index  # The unique levels as labels
                user_level_values = user_level_counts.values  # The counts as values

# Display the pie chart
                st.subheader("📈 **Pie-Chart for User's👨‍💻 Experienced Level**")
                fig = px.pie(
                    values=user_level_values, 
